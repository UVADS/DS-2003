...
ui <- fluidPage(
  ...
  sliderInput("bw_adjust", label = "Span adjustment:",
...
```{r}
ui <- fluidPage(
  # Independent Input Objects
  selectInput("n_breaks2", label = "Number of bins:",
              choices = c(10, 20, 35, 50), selected = 20),
  # Dependent input Objects (created as GUI output objects)
...
server <- function(input, output){
  ...
  # Update dependent Input object
  output$bw_adjust_ui <- renderUI({
    # Prevents errors when app renders at start
      if(is.null(input$n_breaks2)) { return()}
    
     sliderInput("bw_adjust2", label = "Bandwidth adjustment:",
              min = 0.2, max = as.numeric(input$n_breaks2)/10, 
              value = (0.2 + as.numeric(input$n_breaks2)/10)/2, step = 0.2)
    })

  # Update Output object
    output$irisplot <- renderPlot({
     # Prevents errors when app renders at start
     if(is.null(input$n_breaks2) || is.null(input$bw_adjust2)){ return() }
...
## without reactive expressions
 ...
 - input noise to mathpre2 refreshed when 'input$span' *or*() 'input$sd' is changed
...
server <- function(input, output){
   ...
   output$scsplot <- renderPlot({
    ...
    ggplot(scs, aes(x=mars, y=mathpre2)) + geom_point(alpha=0.3) + 
      geom_smooth(method="loess", formula=y~x, se=FALSE, span=input$span) + 
      theme_bw() + ylim(-1, 20) + xlim(20, 130)
    })}
...
## with reactive expressions
 ...
 - input noise to mathpre2 refreshed when 'input$sd2' is changed, but not when 
 'input$span2' is changed
...
server <- function(input, output){
  ...
  # Only executes when input$sd2 is changed
    scs_data <- reactive({
     scs$mathpre2 <- scs$mathpre + rnorm(nrow(scs), mean = 0, sd = input$sd2) # add noise
     scs
    })
      
    # Only executes when input$span2 is changed
   output$scsplot <- renderPlot({
      ggplot(scs_data(), aes(x=mars, y=mathpre2)) + geom_point(alpha=0.3) + 
       geom_smooth(method="loess", formula=y~x, se=FALSE, span=input$span2)  +
       theme_bw() + ylim(-1, 20) + xlim(22, 128)
     })}
...
server <- function(input, output){
    ...
    ggplot(iris, aes(x=Sepal.Length, y=Sepal.Width)) + geom_point() +  
     geom_smooth(method="loess", formula=y~x, se=FALSE, span=input$span_adj) + 
        theme_bw()
    })
...
server <- function(input, output){
    ...
    output$irisplot <- renderPlot({
    ggplot(iris, aes(x=Sepal.Length, y=Sepal.Width)) + geom_point() +  
     geom_smooth(method="loess", formula=y~x, se=FALSE, span=input$span_adj2) + 
        theme_bw()
    })
...
server <- function(input, output){
    ...
    output$irisplot <- renderPlot({
    ggplot(iris, aes(x=Sepal.Length, y=Sepal.Width)) + geom_point() +  
     geom_smooth(method="loess", formula=y~x, se=FALSE, span=input$span_adj3) + 
        theme_bw()
    })
   ...
   output$scsplot <- renderPlot({
      ggplot(scs_data(), aes(x=mars, y=mathpre2)) + geom_point(alpha=0.3) + 
       geom_smooth(method="loess", formula=y~x, se=FALSE, span=input$span3)  + 
       theme_bw() + ylim(-1, 20) + xlim(22, 128)
     })
}
...
server <- function(input, output){
  ...
  output$scsplot2 <- renderPlot({
      ggplot(scs_data2(), aes(x=mars, y=mathpre2)) + geom_point(alpha=0.3) + 
      geom_smooth(method="loess", formula=y~x, se=FALSE, span=input$span5, col="red")  + 
      theme_bw() + ylim(-1, 20) + xlim(22, 128)
     })
}
...
shinyApp(
  ...
  server = function(input, output){
    ...
    p <- ggplot(iris, aes(x=Sepal.Length, y=Sepal.Width)) + geom_point() + 
      geom_smooth(method="loess", formula=y~x, se=FALSE, span=input$span_p) + 
      theme_bw()
