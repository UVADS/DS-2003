...
planets_df <- data.frame(name = name, type = type, diameter, rotation, rings)
...
mpg_10_v2 <- mtcars[ mtcars$mpg > 10, ]
mpg_10_v2
...
subset(x = mtcars, subset = c(cyl == 4, mpg > 20), select = c(cyl, mpg, hp))

subset(mtcars, (cyl == 4) & (mpg > 20), c(cyl, mpg, hp))
...
```{r}
# read in data
incex <- read.table('income_exmpl.dat', header = T, sep = "\t") # use (..) to move up a directory
