...
class title: "Descriptive Statistics and Simple Plotting in R"
class : "6"
---
...
colMedians <- apply(mtcars,         # data object
                    MARGIN = 1,     # operate on columns (specified margin),
                    FUN = median)   # function to execute
...
## CROSSTABS
```{r}
#How to calculate mean of elements in a vector split into different subgroups
# Cross tabulation method compared to tapply
...
tapply(mtcars$mpg, mtcars$cyl, sum)           
 ...
 # mean mpg by cyl
xtabs(mpg ~ cyl, data = mtcars) / xtabs( ~ cyl, data = mtcars)
...
boxplot(mpg ~ cyl, data = mtcars, varwidth = F, horizontal = F) #default

boxplot(mpg ~ cyl, data = mtcars, varwidth = T, horizontal = F)
...
my_summary <- function(x)
{
   ...
   return(c(mean = m, std.dev = s, n = n, s.e. = se))
}
