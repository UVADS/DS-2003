...
```{r}
# Convert edu from numeric to factor
income$fedu <- factor(income$edu, levels = c('low', 'med','high'))
...
## Student_____________________________________________________________________
* Use shape to denote participant's biological sex
...
ggplot(income, aes(age, income, color = fedu)) +       
  geom_point(aes(shape = sex, alpha = oexp)) + theme_classic() 

ggplot(income, aes(oexp, income, color = fedu)) +       
...
# This dplyr sequence uses gather to convert a wide dataset to a long
mtcars_summary_long <- mtcars %>%
  select(fcyl, wt) %>%
...
# Alternative dplyr commands without gather function
mtcars_summary_long <- mtcars %>%
  select(fcyl, wt) %>%
  group_by(fcyl) %>%
